...
public class BoardMenu {
    ...
    private void blockCard()throws SQLException {
        ...
        var boardColumnsInfo = boardEntity.getBoardColumns().stream()
                .map(bc-> new BoardColumnInfoDTO(bc.getId(),bc.getOrder(),bc.getKind()))
                .toList();
        try(var connetion = getConnection()){
            new CardService(connetion).block(cardId, reason, boardColumnsInfo);
        }catch(RuntimeException ex){
            System.out.println(ex.getMessage());
        }
    }
    ...
    private void unblockCard() throws SQLException{
        ...
        var reason = scanner.next();
        try(var connetion = getConnection()){
            new CardService(connetion).unblock(cardId, reason);
        }catch(RuntimeException ex){
            System.out.println(ex.getMessage());
        }
    }
        ...
        try(var connection = getConnection()){
            new CardQueryService(connection).findById(selectedCardId)
                    .ifPresentOrElse(c -> {
                        ...
                        System.out.println(c.blocked()? "Está bloqueado. Motivo: %s" + c.blockReason() :
                                "Não está bloqueado");
